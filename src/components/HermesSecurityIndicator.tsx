 ...
 * Se muestra en la esquina inferior derecha — SOLO para administradores
 */
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-10 h-10 sm:w-14 sm:h-14 rounded-full shadow-2xl flex items-center justify-center transition-all ${
          hasIssues
      ...
      >
        ...
        {hasIssues && (
          <span className="absolute -top-1 -right-1 w-3 h-3 sm:w-4 sm:h-4 bg-red-500 rounded-full border-2 border-white animate-ping" />
        )}
      </motion.button>
      ...
      <AnimatePresence>
        {show && (
          <motion.div
            ...
            className="fixed bottom-16 right-4 sm:bottom-24 sm:right-6 z-50 w-[calc(100vw-2rem)] max-w-80 bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden"
          >
            {/* Header */}
 ...
 * El indicador solo se muestra si el usuario es admin
 */
